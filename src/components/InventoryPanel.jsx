import { vehicles } from '../game/content'

const money = (value) => `$${value.toLocaleString()}`

export default function InventoryPanel({ game, onBuy, onSelect }) {
  const available = vehicles.filter((vehicle) => !game.inventory.some((item) => item.id === vehicle.id))
  return (
    <section className="panel inventory-panel">
      ...
      <div className="vehicle-list">
        ...
        {!available.length && <p className="empty-state">You have acquired every starter listing. Sell one to make room for another.</p>}
      </div>
